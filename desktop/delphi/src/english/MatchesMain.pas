...
uses
  ...
  Buttons, ASubject, Vcl.ExtCtrls, Vcl.Imaging.jpeg;
...
begin
  ...
  if Matches.Count = 0 then begin
    ShowMessage('Sorry, but you lost... :(');
    NewGame;
    Exit;
  end;
...
begin
  ShellExecute(Handle, 'open', PChar(TLabel(Sender).Caption){'http://www.julfysoft.orgfree.com/'}, nil, nil, SW_SHOWNORMAL);
end;
